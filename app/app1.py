...
def summarize_selects(unit: Unit) -> Dict[str, Any]:
    ...
    for s in unit.selects:
        ...
        # Flagging SELECT/INSERT/UPDATE/MODIFY as before
        if tbl_upper in OLD_TO_NEW_TABLE_MAP and s.target_type.upper() in {"SELECT", "INSERT", "UPDATE", "MODIFY"}:
        ...
        # ---- EXTRA: Flag DATA/TYPE as additional risk for assessment/prompt
        if tbl_upper in OLD_TO_NEW_TABLE_MAP and s.target_type.upper() in {"DATA", "TYPE"}:
            mapping = OLD_TO_NEW_TABLE_MAP[tbl_upper]
            type_data_decls.append({
                "table": s.table,
                "target": s.target_name,
                "declaration_type": s.target_type.upper(),
                "reason": (
                    f"{s.target_type.upper()} declaration using {tbl_upper} found. "
                    f"Replace with {mapping['new_table']} (for KNA1/LFA1) per SAP Note 2877717 and review structure mapping for differences."
                )
            })
    ...
    # Only add if something found (keeps response unchanged for old cases)
    if type_data_decls:
        stats["legacy_declarations"] = type_data_decls
...
1) Produce a concise **assessment** highlighting:
   ...
   - Point out any DATA/TYPE declaration using J_1IMOCUST or J_1IMOVEND and recommend replacing it with KNA1 or LFA1; warn if structure mapping is needed.
